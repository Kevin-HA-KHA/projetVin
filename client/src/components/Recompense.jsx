...
function Recompense() {
    ...
    // useEffect(() => {
    //     const localURL = '/wineDataRecompense.json';
    //     const backupURL = '../src/assets/data/wineDataRecompense.json';
    ...
    useEffect(() => {
        const fetchEvent = async () => {
          try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/wine/recompense`); 
            const data = await response.json();
            setWines(data);
            setSelectedWine(data[0]); 
          } catch (error) {
